...
class TaxReportView(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Label(form_frame, text="Loại Vật Liệu:").grid(row=2, column=0, sticky="w", pady=4)
        self.cbo_product = ttk.Combobox(form_frame, width=20, state="readonly")
        self.cbo_product.grid(row=2, column=1, pady=4, sticky="ew")

        ttk.Label(form_frame, text="Khối Lượng Mua (m³):").grid(row=3, column=0, sticky="w", pady=4)
        self.ent_volume = ttk.Entry(form_frame, width=22)
        self.ent_volume.grid(row=3, column=1, pady=4, sticky="ew")

        ttk.Label(form_frame, text="Tổng Tiền Hóa Đơn (đ):").grid(row=4, column=0, sticky="w", pady=4)
        self.ent_amount = ttk.Entry(form_frame, width=22)
        self.ent_amount.grid(row=4, column=1, pady=4, sticky="ew")
        
        self.lbl_amount_text = tk.Label(form_frame, text="", font=("Segoe UI", 9, "italic"), fg="#059669")
        self.lbl_amount_text.grid(row=4, column=2, sticky="w", padx=5)
        ...
        ttk.Label(form_frame, text="Ghi Chú:").grid(row=5, column=0, sticky="w", pady=4)
        self.ent_note = ttk.Entry(form_frame, width=22)
        self.ent_note.grid(row=5, column=1, pady=4, sticky="ew")

        tk.Button(form_frame, text=" LƯU HÓA ĐƠN", bg=config.COLOR_SUCCESS, fg="white", font=("Segoe UI", 10, "bold"), pady=6, command=self.save_invoice).grid(row=6, column=0, columnspan=2, sticky="ew", pady=15)
        ...
        self.lbl_warning.pack(fill="x", pady=10)
        ...
        tk.Button(bulk_toolbar, text="📂 Chọn File XML", bg="#475569", fg="white", font=("Segoe UI", 9, "bold"), command=self.select_xml_files).pack(side="left", padx=5)
        tk.Label(bulk_toolbar, text="Phím tắt: [Ctrl+Shift+S] Lưu toàn bộ lưới | [F4] Danh sách chi phí chờ").pack(side="right", padx=5)
        
        cols_xml = ("filename", "tax_code", "hs_code", "total_before", "vat_rate", "total_amount", "status")
        ...
        self.grid_xml.heading("hs_code", text="Mã Vật Liệu (Gõ vào đây)")
        self.grid_xml.heading("total_before", text="Tiền Trước Thuế")
        self.grid_xml.heading("vat_rate", text="Thuế Suất VAT (%)")
        self.grid_xml.heading("total_amount", text="Tổng Tiền")
        self.grid_xml.heading("status", text="Trạng Thái")
        
        self.grid_xml.column("filename", width=150)
        self.grid_xml.column("tax_code", width=100)
        self.grid_xml.column("hs_code", width=150)
        self.grid_xml.column("total_before", width=120, anchor="e")
        self.grid_xml.column("vat_rate", width=100, anchor="center")
        self.grid_xml.column("total_amount", width=120, anchor="e")
        self.grid_xml.column("status", width=250)
    ...
    def save_invoice(self):
        ...
        if not code or not date or idx < 0 or not vol or not amt:
            messagebox.showwarning("Thiếu thông tin", "Vui lòng nhập đầy đủ Số HĐ, Ngày, Loại, Khối lượng và Tổng tiền!")
            return
        ...
        try:
            ...
            if invoice_dao.add_invoice(code, date, pid, vol, amt, note):
                messagebox.showinfo("Thành công", f"Đã lưu hóa đơn đầu vào {code} thành công!")
    ...
    def select_xml_files(self):
        files = filedialog.askopenfilenames(
            title="Chọn các file Hóa Đơn Điện Tử (XML)",
        ...
        if files:
            ...
            for data in parsed_data:
                self.grid_xml.insert("", "end", values=(
                    data["filename"],
                    data["tax_code"],
        ...
        if column == "#3":
            ...
            def save_edit(e):
                ...
                if new_val:
                    ...
                    self.grid_xml.set(item, "#7", "Đang tra cứu API thuế suất...")
    ...
    def mock_hs_api_call(self, item, hs_code):
        ...
        def update_ui():
            try:
                self.grid_xml.set(item, "#5", f"{vat}")
                self.grid_xml.set(item, "#7", "Đã cập nhật thuế suất thành công")
            except Exception:
    ...
    def save_bulk_invoices(self):
        ...
        for item in children:
            self.grid_xml.set(item, "#7", "Đã lưu & cấn trừ tự động!")
        
        messagebox.showinfo("Lưu Thành Công", f"Đã tự động cấn trừ thuế cho {len(children)} hóa đơn!")
        self.load_summary()
