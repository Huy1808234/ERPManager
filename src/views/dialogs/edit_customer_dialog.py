...
class EditCustomerDialog(tk.Toplevel):
    def __init__(self, parent, customer, on_success_callback=None):
    ...
    def setup_ui(self):
        ttk.Label(self, text="Cập Nhật Hồ Sơ Khách Hàng", font=("Segoe UI", 12, "bold"), foreground="#d97706").pack(pady=10)
        ...
        self.ent_name.insert(0, self.customer['name'])
        ...
        self.ent_phone.insert(0, self.customer['phone'] or "")
        ...
        self.ent_addr.insert(0, self.customer['address'] or "")
        ...
        self.ent_limit.insert(0, str(int(self.customer['credit_limit'])))
        self.ent_limit.grid(row=3, column=1, pady=3)

        ttk.Label(form, text="Bằng chữ:").grid(row=4, column=0, sticky="w", pady=3)
        self.lbl_limit_words = ttk.Label(form, text=num2vietnamese_words(self.customer['credit_limit']), font=("Segoe UI", 9, "italic", "bold"), foreground="#2563eb")
        ...
        self.var_is_contractor = tk.IntVar(value=self.customer['is_contractor'])
        ...
        tk.Button(self, text="Lưu Thay Đổi", bg="#d97706", fg="white", font=("Segoe UI", 10, "bold"), command=self.update_customer).pack(pady=10)
